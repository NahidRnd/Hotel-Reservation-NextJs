...
const HotelsPage = async (props: {searchParams:Promise<{destination: string, date: Range, options: OptionsSelect, price: string, amenities: string[]}>}) => {
    const {destination, date={startDate: new Date(), endDate: new Date(Date.now() + 86400000)}, options={Adult:1, children:0, room:1}, price="0,500", amenities} = await props.searchParams;
